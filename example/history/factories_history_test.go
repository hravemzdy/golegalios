	...
	for testYear := minYear; testYear <= maxYear; testYear++ {
		...
		for testMonth := 2; testMonth <= 12; testMonth++ {
			...
			if testHealthNext.MinMonthlyBasis() != testHealthProp.MinMonthlyBasis() {
				MES_HEALTH_MIN_MONTHLY_BASIS = testIntMonth
			}
			if testHealthNext.MaxAnnualsBasis() != testHealthProp.MaxAnnualsBasis() {
				MES_HEALTH_MAX_ANNUALS_BASIS = testIntMonth
			}
			if testHealthNext.LimMonthlyState() != testHealthProp.LimMonthlyState() {
				MES_HEALTH_LIM_MONTHLY_STATE = testIntMonth
			}
			if testHealthNext.LimMonthlyDis50() != testHealthProp.LimMonthlyDis50() {
				MES_HEALTH_LIM_MONTHLY_DIS50 = testIntMonth
			}
			if testHealthNext.FactorCompound().Equal(testHealthProp.FactorCompound()) == false {
				MES_HEALTH_FACTOR_COMPOUND = testIntMonth
			}
			if testHealthNext.FactorEmployee().Equal(testHealthProp.FactorEmployee()) == false {
				MES_HEALTH_FACTOR_EMPLOYEE = testIntMonth
			}
			if testHealthNext.MarginIncomeEmp() != testHealthProp.MarginIncomeEmp() {
				MES_HEALTH_MARGIN_INCOME_EMP = testIntMonth
			}
			if testHealthNext.MarginIncomeAgr() != testHealthProp.MarginIncomeAgr() {
				MES_HEALTH_MARGIN_INCOME_AGR = testIntMonth
			}
			if testSalaryNext.WorkingShiftWeek() != testSalaryProp.WorkingShiftWeek() {
				MES_SALARY_WORKING_SHIFT_WEEK = testIntMonth
			}
			if testSalaryNext.WorkingShiftTime() != testSalaryProp.WorkingShiftTime() {
				MES_SALARY_WORKING_SHIFT_TIME = testIntMonth
			}
			if testSalaryNext.MinMonthlyWage() != testSalaryProp.MinMonthlyWage() {
				MES_SALARY_MIN_MONTHLY_WAGE = testIntMonth
			}
			if testSalaryNext.MinHourlyWage() != testSalaryProp.MinHourlyWage() {
				MES_SALARY_MIN_HOURLY_WAGE = testIntMonth
			}
			if testSocialNext.MaxAnnualsBasis() != testSocialProp.MaxAnnualsBasis() {
				MES_SOCIAL_MAX_ANNUALS_BASIS = testIntMonth
			}
			if testSocialNext.FactorEmployer().Equal(testSocialProp.FactorEmployer()) == false {
				MES_SOCIAL_FACTOR_EMPLOYER = testIntMonth
			}
			if testSocialNext.FactorEmployerHigher().Equal(testSocialProp.FactorEmployerHigher()) == false {
				MES_SOCIAL_FACTOR_EMPLOYER_HIGHER = testIntMonth
			}
			if testSocialNext.FactorEmployee().Equal(testSocialProp.FactorEmployee()) == false {
				MES_SOCIAL_FACTOR_EMPLOYEE = testIntMonth
			}
			if testSocialNext.FactorEmployeeReduce().Equal(testSocialProp.FactorEmployeeReduce()) == false {
				MES_SOCIAL_FACTOR_EMPLOYEE_REDUCE = testIntMonth
			}
			if testSocialNext.FactorEmployeeGarant().Equal(testSocialProp.FactorEmployeeGarant()) == false {
				MES_SOCIAL_FACTOR_EMPLOYEE_GARANT = testIntMonth
			}
			if testSocialNext.MarginIncomeEmp() != testSocialProp.MarginIncomeEmp() {
				MES_SOCIAL_MARGIN_INCOME_EMP = testIntMonth
			}
			if testSocialNext.MarginIncomeAgr() != testSocialProp.MarginIncomeAgr() {
				MES_SOCIAL_MARGIN_INCOME_AGR = testIntMonth
			}
			if testTaxingNext.AllowancePayer() != testTaxingProp.AllowancePayer() {
				MES_TAXING_ALLOWANCE_PAYER = testIntMonth
			}
			if testTaxingNext.AllowanceDisab1st() != testTaxingProp.AllowanceDisab1st() {
				MES_TAXING_ALLOWANCE_DISAB_1ST = testIntMonth
			}
			if testTaxingNext.AllowanceDisab2nd() != testTaxingProp.AllowanceDisab2nd() {
				MES_TAXING_ALLOWANCE_DISAB_2ND = testIntMonth
			}
			if testTaxingNext.AllowanceDisab3rd() != testTaxingProp.AllowanceDisab3rd() {
				MES_TAXING_ALLOWANCE_DISAB_3RD = testIntMonth
			}
			if testTaxingNext.AllowanceStudy() != testTaxingProp.AllowanceStudy() {
				MES_TAXING_ALLOWANCE_STUDY = testIntMonth
			}
			if testTaxingNext.AllowanceChild1st() != testTaxingProp.AllowanceChild1st() {
				MES_TAXING_ALLOWANCE_CHILD_1ST = testIntMonth
			}
			if testTaxingNext.AllowanceChild2nd() != testTaxingProp.AllowanceChild2nd() {
				MES_TAXING_ALLOWANCE_CHILD_2ND = testIntMonth
			}
			if testTaxingNext.AllowanceChild3rd() != testTaxingProp.AllowanceChild3rd() {
				MES_TAXING_ALLOWANCE_CHILD_3RD = testIntMonth
			}
			if testTaxingNext.FactorAdvances().Equal(testTaxingProp.FactorAdvances()) == false {
				MES_TAXING_FACTOR_ADVANCES = testIntMonth
			}
			if testTaxingNext.FactorWithhold().Equal(testTaxingProp.FactorWithhold()) == false {
				MES_TAXING_FACTOR_WITHHOLD = testIntMonth
			}
			if testTaxingNext.FactorSolidary().Equal(testTaxingProp.FactorSolidary()) == false {
				MES_TAXING_FACTOR_SOLIDARY = testIntMonth
			}
			if testTaxingNext.FactorTaxRate2().Equal(testTaxingProp.FactorTaxRate2()) == false {
				MES_TAXING_FACTOR_TAXRATE2 = testIntMonth
			}
			if testTaxingNext.MinAmountOfTaxBonus() != testTaxingProp.MinAmountOfTaxBonus() {
				MES_TAXING_MIN_AMOUNT_OF_TAXBONUS = testIntMonth
			}
			if testTaxingNext.MaxAmountOfTaxBonus() != testTaxingProp.MaxAmountOfTaxBonus() {
				MES_TAXING_MAX_AMOUNT_OF_TAXBONUS = testIntMonth
			}
			if testTaxingNext.MarginIncomeOfTaxBonus() != testTaxingProp.MarginIncomeOfTaxBonus() {
				MES_TAXING_MARGIN_INCOME_OF_TAXBONUS = testIntMonth
			}
			if testTaxingNext.MarginIncomeOfRounding() != testTaxingProp.MarginIncomeOfRounding() {
				MES_TAXING_MARGIN_INCOME_OF_ROUNDING = testIntMonth
			}
			if testTaxingNext.MarginIncomeOfWithhold() != testTaxingProp.MarginIncomeOfWithhold() {
				MES_TAXING_MARGIN_INCOME_OF_WITHHOLD = testIntMonth
			}
			if testTaxingNext.MarginIncomeOfSolidary() != testTaxingProp.MarginIncomeOfSolidary() {
				MES_TAXING_MARGIN_INCOME_OF_SOLIDARY = testIntMonth
			}
			if testTaxingNext.MarginIncomeOfTaxRate2() != testTaxingProp.MarginIncomeOfTaxRate2() {
				MES_TAXING_MARGIN_INCOME_OF_TAXRATE2 = testIntMonth
			}
			if testTaxingNext.MarginIncomeOfWthEmp() != testTaxingProp.MarginIncomeOfWthEmp() {
				MES_TAXING_MARGIN_INCOME_OF_WHT_EMP = testIntMonth
			}
			if testTaxingNext.MarginIncomeOfWthAgr() != testTaxingProp.MarginIncomeOfWthAgr() {
				MES_TAXING_MARGIN_INCOME_OF_WHT_AGR = testIntMonth
			}
